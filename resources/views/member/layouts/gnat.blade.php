    ...
    <div class="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-8 lg:flex-row lg:gap-8 lg:py-10">
        <aside class="lg:w-60 lg:rounded-2xl lg:border lg:border-[#351c42]/10 lg:bg-white/80 lg:p-4 lg:shadow-lg lg:shadow-[#351c42]/5">
            <p class="mb-3 text-[0.65rem] font-bold uppercase tracking-widest text-[#965995]">
                @if(!$gnatCanSeeMembership)
                    Account
                @elseif(!$gnatHasActiveSubscription)
                    Membership
                @else
                    Menu
                @endif
            ...
            <nav class="flex flex-col gap-1" aria-label="Member">
                @if($gnatPortalUnlocked)
                    ...
                    <a href="{{ route('member.events.index') }}" class="md-sidebar-link {{ request()->routeIs('member.events.*') ? 'is-active' : '' }}"><span class="h-1.5 w-1.5 rounded-full {{ request()->routeIs('member.events.*') ? 'bg-[#965995]' : 'bg-slate-300' }}"></span> Events</a>
                    <a href="{{ route('member.nominations.index') }}" class="md-sidebar-link {{ request()->routeIs('member.nominations.*') ? 'is-active' : '' }}"><span class="h-1.5 w-1.5 rounded-full {{ request()->routeIs('member.nominations.*') ? 'bg-[#965995]' : 'bg-slate-300' }}"></span> Nominations</a>
                    <a href="{{ route('member.jobs.index') }}" class="md-sidebar-link {{ request()->routeIs('member.jobs.*') ? 'is-active' : '' }}"><span class="h-1.5 w-1.5 rounded-full {{ request()->routeIs('member.jobs.*') ? 'bg-[#965995]' : 'bg-slate-300' }}"></span> Search jobs</a>
