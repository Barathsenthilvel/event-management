<div class="mt-8 border-t border-[#351c42]/10 pt-4" x-data="{ logoutOpen: false }">
    <form x-ref="memberLogoutForm" method="POST" action="{{ route('member.logout') }}" class="hidden">@csrf</form>
    <button
        type="button"
    ...
    {{-- Teleport so position:fixed is relative to the viewport (sidebar transform would trap the overlay otherwise). --}}
