...
>
    ...
    <nav class="flex flex-col gap-1" aria-label="Member">
        @if($showFullMemberMenu)
            ...
            <a href="{{ route('member.jobs.index') }}" class="md-sidebar-link {{ request()->routeIs('member.jobs.*') ? 'is-active' : '' }}" data-md-nav><span class="h-1.5 w-1.5 rounded-full {{ request()->routeIs('member.jobs.*') ? 'bg-[#965995]' : 'bg-[#351c42]/25' }}"></span> Search jobs</a>
