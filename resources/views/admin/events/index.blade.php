            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between w-full md:w-auto md:flex-1 md:min-w-0 md:max-w-3xl">
                <form method="GET" class="flex items-center gap-2 w-full sm:max-w-md min-w-0">
                    <div class="relative flex-1 min-w-0">
                        <input type="search" name="q" value="{{ $q }}" placeholder="Search events..."
                ...
                <div class="flex shrink-0 justify-end sm:justify-start">
                    <a href="{{ route('admin.events.create') }}" class="inline-flex px-4 py-2 rounded-xl bg-indigo-600 text-white text-xs font-extrabold">+ Create Event</a>
                </div>
            </div>
        </div>
    </div>
                                    ...
                                    <x-admin.row-actions>
                                        ...
                                        <x-slot:more>
                                            ...
                                            @if($event->is_active && in_array($event->status, ['upcoming', 'live'], true))
                                                <a href="{{ route('admin.events.invite', ['event' => $event->id, 'reminder' => 1]) }}"
