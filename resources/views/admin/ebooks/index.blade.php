                ...
                <tr>
                    <td class="px-6 py-4 align-middle">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="font-semibold text-slate-900">{{ $ebook->title }}</p>
                    ...
                    <td class="px-6 py-4 text-right align-middle">
                        <x-admin.row-actions>
                            <x-slot:primary>
                                <a href="{{ route('admin.ebooks.edit', $ebook->id) }}" title="Edit"
                                ...
                                <form method="POST" action="{{ route('admin.ebooks.toggle-status', $ebook->id) }}" class="inline-flex">
                                    @csrf
                                    <button type="submit" title="Toggle display status"
                                        class="w-8 h-8 rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50 inline-flex items-center justify-center">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            ...
                            <x-slot:more>
                                <div class="border-t border-slate-100">
                                    <form id="admin-delete-ebook-{{ $ebook->id }}" method="POST" action="{{ route('admin.ebooks.destroy', $ebook->id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            data-delete-form="admin-delete-ebook-{{ $ebook->id }}"
