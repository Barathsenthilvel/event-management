...
<div class="h-full flex gap-3 workspace-transition relative p-6" x-data="membershipPage()">
                        ...
                        <template x-for="item in filtered" :key="item.id">
                            <tr class="membership-row transition-all">
                                ...
                                <td class="px-6 py-4 bg-white border-y border-r border-slate-100 rounded-r-2xl text-right">
                                    <x-admin.row-actions>
                                        <x-slot:primary>
                                            <button type="button" @click="openPanel('edit', item)"
                                                class="w-8 h-8 rounded-lg border border-slate-200 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 inline-flex items-center justify-center" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                            </button>
                                            <button type="button" @click="confirmDelete(item)"
                                                class="w-8 h-8 rounded-lg bg-rose-600 text-white hover:bg-rose-700 inline-flex items-center justify-center" title="Delete">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                            </button>
                                        </x-slot:primary>
                                    </x-admin.row-actions>
                                </td>
    ...
    <div x-show="showPanel" x-cloak
        ...
        <form :action="isEditing ? form.update_url : '{{ route('admin.memberships.store') }}'"
            method="POST"
