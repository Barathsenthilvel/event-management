            ...
            <tbody class="text-xs">
                ...
                <tr class="group transition-all hover:bg-slate-50/50">
                    ...
                    <td class="px-4 py-3 bg-white border-y border-r border-slate-100 last:rounded-r-2xl text-right align-middle">
                        <x-admin.row-actions class="opacity-0 group-hover:opacity-100 transition-opacity">
                            <x-slot:primary>
                                ...
                                @if(!$admin->is_super_admin)
                                    <form id="admin-delete-admin-{{ $admin->id }}-table" action="{{ route('admin.admins.destroy', $admin) }}" method="POST" class="inline-flex">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
