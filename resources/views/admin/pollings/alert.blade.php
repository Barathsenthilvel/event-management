...
<div class="flex-1 overflow-y-auto custom-scroll p-6">
    <div class="max-w-6xl mx-auto space-y-5">
        <div class="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-extrabold text-slate-900">GNAT Live Polling Alert</h1>
                <p class="text-xs font-bold text-slate-500 mt-1">Polling: {{ $polling->title }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <form method="GET" action="{{ route('admin.pollings.alert', $polling) }}" class="flex items-center gap-2">
                ...
                <a href="{{ route('admin.pollings.index') }}" class="px-4 py-2 rounded-xl border border-slate-300 text-xs font-extrabold text-slate-700">Back</a>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('admin.pollings.alert.store', $polling->id) }}" class="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-5">
            @csrf
                ...
                <div>
                    ...
                    <div class="mb-2 flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                        <label class="inline-flex cursor-pointer items-center gap-2 text-xs font-extrabold text-slate-800">
                            <input type="checkbox" id="polling-alert-select-all-page" class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                    ...
                    <div class="max-h-80 overflow-y-auto border border-slate-100 rounded-xl divide-y divide-slate-100">
                        @foreach($members as $m)
                            ...
                            <label class="flex items-start justify-between gap-3 p-3 hover:bg-slate-50">
                                ...
                                <input type="checkbox" name="member_ids[]" value="{{ $m->id }}" class="js-polling-alert-member-cb h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" {{ $checked ? 'checked' : '' }}>
                            </label>
                        @endforeach
                    </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-extrabold">Send alert</button>
                </div>
            </div>
        </form>
...
(function () {
    function memberCbs() {
        return Array.prototype.slice.call(document.querySelectorAll('.js-polling-alert-member-cb'));
    }
    function syncSelectAllMaster() {
        var master = document.getElementById('polling-alert-select-all-page');
        if (!master) return;
        var cbs = memberCbs();
        if (cbs.length === 0) {
