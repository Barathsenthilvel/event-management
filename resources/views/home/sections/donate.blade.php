        ...
        <div class="mx-auto max-w-7xl px-4">
            ...
            <div class="overflow-hidden -mx-0" data-donate-viewport>
                <div class="flex gap-4 will-change-transform transition-transform duration-500 ease-out" data-donate-track>
                    @forelse ($homeDonations ?? collect() as $donation)
                            @php
                                $coverSrc = $donation->cover_image_path
                                    ? asset('storage/' . $donation->cover_image_path)
                                    : 'data:image/svg+xml,' . rawurlencode('<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300"><rect fill="#e8e3dc" width="100%" height="100%"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#965995" font-family="sans-serif" font-size="14">GNAT Donation</text></svg>');
                    ...
                    @empty
                        <div class="w-full rounded-3xl border border-dashed border-[#351c42]/20 bg-white px-6 py-12 text-center">
                            <p class="text-sm font-bold text-[#351c42]/80">No featured campaigns right now.</p>
                            <p class="mt-2 text-sm text-[#351c42]/60">Check back soon or browse all donation programs.</p>
                            <a href="{{ route('donations.index') }}" class="mt-4 inline-flex items-center justify-center rounded-2xl bg-[#351c42] px-5 py-2.5 text-sm font-extrabold text-[#fddc6a] hover:bg-[#4d2a5c] transition">
                                View all donations
                            </a>
                        </div>
                    @endforelse
