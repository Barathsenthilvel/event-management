                ...
                <div id="testimonialStackScroll" class="hidden relative min-h-[520px] overflow-hidden">
                    <div class="sticky top-6">
                        <div id="testimonialStack" class="relative w-full">
                            @foreach ($testimonial_stack_cards as $i => $t)
                                <article class="stack-card rounded-3xl bg-[#351c42] shadow-sm overflow-hidden border border-white/10" style="--i:{{ $i }}">
                                        ...
                                        <div class="relative mx-auto w-full max-w-sm">
                                            <div class="absolute -left-12 -top-10 rounded-2xl bg-white/20 backdrop-blur-sm border border-white/10 p-2">
                                                <div class="relative h-44 w-36 overflow-hidden rounded-xl">
                                                    <img src="{{ asset($t['image']) }}" alt="Client" class="h-full w-full object-cover" />
                                                    @if (!empty($t['play']))
                                            ...
                                            <div class="rounded-2xl border border-white/10 bg-white/5 p-6">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <span class="text-white text-sm font-medium">Rating {{ $t['rating'] }}</span>
                                                </div>

                                                <p class="mt-5 text-white/90 text-sm leading-6">{{ $t['quote'] }}</p>

                                                <div class="mt-5">
                                                    <div class="text-white font-semibold text-sm">{{ $t['name'] }}</div>
                                                    <div class="text-white/70 text-xs mt-1">{{ $t['role'] }}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
