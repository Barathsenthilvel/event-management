...
<div x-data="{
    ...
    panelMode: 'create',
    selectedAdmin: null,
    roles: <?php echo \Illuminate\Support\Js::from($roles)->toHtml() ?>,
    adminRoles: [],
    loading: false,
    searchQuery: '',
    addToast(msg) {
...
<div class="flex flex-col gap-3 workspace-transition flex-1" :class="showAddPanel ? 'w-2/3' : 'w-full'">
    ...
    <div class="flex-1 overflow-y-auto custom-scroll pr-2 pb-4">
                ...
                <tr class="group transition-all hover:bg-slate-50/50">
                    ...
                    <td class="px-4 py-3 bg-white border-y border-slate-100 align-middle">
                        <?php $__empty_2 = true; $__currentLoopData = $admin->roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_2 = false; ?>
                            <span class="px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-medium rounded"><?php echo e($role->name); ?></span><?php if(!$loop->last): ?>, <?php endif; ?>
                    ...
                    <td class="px-4 py-3 bg-white border-y border-r border-slate-100 last:rounded-r-2xl text-right align-middle">
                        <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button @click="openEditPanel(<?php echo e($admin->id); ?>)"
                            ...
                            <form id="admin-delete-admin-<?php echo e($admin->id); ?>-table" action="<?php echo e(route('admin.admins.destroy', $admin)); ?>" method="POST" class="inline">
                                <?php echo csrf_field(); ?>
                                <?php echo method_field('DELETE'); ?>
                                <button type="button"
                                    data-delete-form="admin-delete-admin-<?php echo e($admin->id); ?>-table"
        ...
        <div class="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between shrink-0">
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Showing <?php echo e($admins->firstItem()); ?> to <?php echo e($admins->lastItem()); ?> of <?php echo e($admins->total()); ?> Users</p>
            <div class="flex gap-1">
                <?php echo e($admins->links('pagination.simple-tailwind')); ?>

            </div>
        </div>
        <?php else: ?>
        <div class="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between shrink-0">
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Showing <?php echo e($admins->count()); ?> Users</p>
        </div>
        <?php endif; ?>
            ...
            <div
                ...
                <div class="flex justify-between items-start mb-4">
                    <div class="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-indigo-500 font-bold group-hover:bg-indigo-500 group-hover:text-white transition-colors">
                        <?php echo e(strtoupper(substr($admin->name, 0, 2))); ?></div>
                    <span
                        class="px-2 py-0.5 rounded bg-emerald-50 text-emerald-600 font-bold text-[9px] uppercase">Active</span>
                </div>
                <h4 class="font-bold text-slate-800 mb-1"><?php echo e($admin->name); ?></h4>
                <p class="text-xs text-slate-400 mb-2"><?php echo e($admin->email); ?></p>
                <div class="flex flex-wrap gap-1 mb-4">
                    <?php $__empty_1 = true; $__currentLoopData = $admin->roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <span class="px-2 py-1 bg-slate-100 text-slate-600 text-[9px] font-bold rounded"><?php echo e($role->name); ?></span>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                ...
                <div class="mt-4 flex gap-2 pt-4 border-t border-slate-50">
                    ...
                    <?php if(!$admin->is_super_admin): ?>
                    <form id="admin-delete-admin-<?php echo e($admin->id); ?>-card" action="<?php echo e(route('admin.admins.destroy', $admin)); ?>" method="POST" class="inline">
                        <?php echo csrf_field(); ?>
                        <?php echo method_field('DELETE'); ?>
                        <button type="button" class="p-2 bg-slate-50 text-slate-400 rounded-xl hover:text-rose-600"
                            data-delete-form="admin-delete-admin-<?php echo e($admin->id); ?>-card"
                            data-delete-title="Delete this admin user?"
                            data-delete-message="They will lose access to the admin panel immediately."
...
<div x-show="showAddPanel" x-cloak
    ...
    <!-- Create form -->
    <form x-show="panelMode === 'create'" method="POST" action="<?php echo e(route('admin.admins.store')); ?>" @submit.prevent="
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\acer\OneDrive\Desktop\projects\event-management\resources\views\admin\admins\index.blade.php ENDPATH**/ ?>