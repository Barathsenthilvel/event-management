<?php $__env->startSection('title', 'Membership plans — GNAT Association'); ?>
...
<div class="space-y-6">
    <div class="rounded-[28px] border border-[#351c42]/10 bg-gradient-to-br from-white via-white to-[#965995]/8 shadow-sm p-6 md:p-7">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl md:text-3xl font-extrabold text-[#351c42] tracking-tight">Hello, <?php echo e($user->first_name ?? 'Member'); ?></h1>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm font-extrabold text-slate-900">
                        <?php echo e($activeSubscription->subscription_type); ?> • <?php echo e(ucfirst(str_replace('_', ' ', (string) $activeSubscription->payment_type))); ?>

                    </p>
                    <p class="mt-1 text-xs font-bold text-emerald-800/80">
                        Valid till: <?php echo e(optional($activeSubscription->end_date)->format('M d, Y')); ?>

                    </p>
                </div>
                <div class="flex gap-3">
                    <a href="<?php echo e(route('member.subscription.index', ['type' => 'Renewal'])); ?>"
        ...
        <div class="xl:col-span-12">
            <div class="bg-white p-6 md:p-8 rounded-[28px] border border-slate-100 shadow-sm">
                <div class="flex items-center justify-between gap-4 mb-6">
                    <div>
                        ...
                        <p class="text-xs text-slate-500">
                            ...
                            <?php if(in_array(($filterType ?? null), ['New','Renewal'], true)): ?>
                                <span class="font-extrabold text-[#965995]">Showing: <?php echo e($filterType); ?></span>
                            <?php endif; ?>
                        </p>
                    </div>
                    <div class="shrink-0">
                        <span class="inline-flex items-center gap-2 rounded-2xl border border-[#965995]/25 bg-[#965995]/10 px-4 py-2 text-[11px] font-extrabold text-[#351c42]">
                            Active plans:
                            <span class="rounded-xl bg-white px-2 py-0.5 text-[#965995] border border-[#965995]/20"><?php echo e((int) ($settingsCount ?? 0)); ?></span>
                        </span>
                    </div>
                </div>
                ...
                <form id="subscription-form" class="space-y-6"
                    x-data="{
                        viewType: 'grid',
                        selectedPlan: <?php echo e(old('membership_setting_id') !== null && old('membership_setting_id') !== '' ? (int) old('membership_setting_id') : 'null'); ?>,
                        pick(id) {
                    ...
                    <div class="sr-only" aria-hidden="true">
                        <?php $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <input type="radio" name="membership_setting_id" id="mp-<?php echo e($p->id); ?>" value="<?php echo e($p->id); ?>"
                                <?php echo e((string) old('membership_setting_id') === (string) $p->id ? 'checked' : ''); ?> />
                    ...
                    <div x-show="viewType === 'grid'" x-cloak class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                        <?php $__empty_1 = true; $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            ...
                            <label for="mp-<?php echo e($plan->id); ?>"
                                class="relative flex flex-col rounded-[20px] border border-[#351c42]/12 bg-white p-5 shadow-sm transition-all cursor-pointer"
                                :class="selectedPlan === <?php echo e((int) $plan->id); ?> ? 'ring-2 ring-[#965995] ring-offset-2 border-[#965995]/50 shadow-md' : 'hover:border-[#965995]/35 hover:shadow-md'"
                                data-payable="<?php echo e($payable); ?>">
                                ...
                                <div class="mt-4 grid grid-cols-2 gap-3">
                                    <div class="rounded-xl border border-slate-200/70 bg-slate-100/70 px-3 py-3">
                                        ...
                                        <p class="mt-1 text-base font-black tabular-nums text-slate-900">₹ <?php echo e(number_format((float) $plan->membership_fee, 2)); ?></p>
                                    </div>
                                    <div class="rounded-xl border border-slate-200/70 bg-slate-100/70 px-3 py-3">
                                        <p class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Reg. fee</p>
                                        <?php if($isNew): ?>
                                            <p class="mt-1 text-base font-black tabular-nums text-slate-900">₹ <?php echo e(number_format((float) $plan->registration_fee, 2)); ?></p>
                                            <?php if($plan->registration_fee_enabled): ?>
                                ...
                                <div class="mt-3 rounded-xl border border-slate-200/70 bg-slate-100/70 px-3 py-3">
                                    ...
                                    <p class="mt-1 text-base font-black text-slate-900"><?php echo e($graceDays); ?> <?php echo e($graceDays === 1 ? 'day' : 'days'); ?></p>
                                </div>
                                ...
                                <div class="mt-3 rounded-xl border border-[#965995]/25 bg-[#965995]/8 px-3 py-2.5 flex items-center justify-between gap-2">
                                    ...
                                    <span class="text-sm font-black tabular-nums text-[#351c42]">₹ <?php echo e(number_format($payable, 2)); ?></span>
                                </div>
                                ...
                                <div class="mt-4 flex gap-2">
                                    <span class="flex flex-1 items-center justify-center rounded-xl bg-[#965995]/15 py-3 text-sm font-black text-[#351c42] transition-colors"
                                        :class="selectedPlan === <?php echo e((int) $plan->id); ?> ? 'bg-[#351c42] text-[#fddc6a]' : ''"
                                        x-text="selectedPlan === <?php echo e((int) $plan->id); ?> ? 'Selected' : 'Select plan'">
                                    </span>
                                    <span class="flex h-[46px] w-[46px] shrink-0 items-center justify-center rounded-xl border border-[#965995]/30 bg-[#965995]/10 text-[#351c42]"
                                        ...
                                        :class="selectedPlan === <?php echo e((int) $plan->id); ?> ? 'border-[#351c42] bg-[#351c42] text-[#fddc6a]' : ''">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <?php $__empty_1 = true; $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    ...
                                    <tr class="subscription-plan-row cursor-pointer transition-colors hover:bg-[#965995]/10"
                                        data-payable="<?php echo e($payable); ?>"
                                        @click="pick(<?php echo e((int) $plan->id); ?>)"
                                        :class="selectedPlan === <?php echo e((int) $plan->id); ?> ? 'bg-[#965995]/15' : ''">
                                        <td class="px-4 py-3 align-middle text-center">
                                            <span class="inline-flex h-5 w-5 items-center justify-center rounded-full border-2 transition-colors"
                                                :class="selectedPlan === <?php echo e((int) $plan->id); ?> ? 'border-[#351c42] bg-[#351c42]' : 'border-slate-300 bg-white'">
                                                <svg class="h-3 w-3 text-white" :class="selectedPlan === <?php echo e((int) $plan->id); ?> ? 'opacity-100' : 'opacity-0'" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                                            </span>
                                        </td>
                                        <td class="px-4 py-3 font-extrabold text-slate-900"><?php echo e($plan->subscription_type); ?></td>
                                        <td class="px-4 py-3 text-slate-700"><?php echo e($paymentLabel); ?></td>
                                        <td class="px-4 py-3 text-right font-bold tabular-nums">₹ <?php echo e(number_format((float) $plan->membership_fee, 0)); ?></td>
                                        <td class="px-4 py-3 text-right font-semibold text-slate-700">
                                            <?php if($isNew): ?>
                                                ₹ <?php echo e(number_format((float) $plan->registration_fee, 0)); ?>
                                        ...
                                        <td class="px-4 py-3 text-center text-slate-600"><?php echo e((int) ($plan->grace_period ?? 0)); ?>d</td>
                                        <td class="px-4 py-3 text-right font-black text-[#351c42] tabular-nums">₹ <?php echo e(number_format($payable, 0)); ?></td>
                                    </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                    <tr>
            ...
            <div class="p-6 space-y-4">
                <div class="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                    ...
                    <p class="mt-1 text-xs font-bold text-slate-600" id="renewal-blocked-message">
                        <?php echo e(session('renewal_blocked_message')); ?>

                    </p>
                </div>
                <?php if(!empty($activeSubscription) && !empty($activeSubscription->end_date)): ?>
                    <div class="rounded-2xl border border-slate-100 bg-white p-4 flex items-center justify-between">
                        ...
                        <p class="text-xs font-extrabold text-slate-900"><?php echo e(optional($activeSubscription->end_date)->format('M d, Y')); ?></p>
                    </div>
                <?php endif; ?>
...
<script>
const postSuccessRedirectUrl = "<?php echo e(route('member.dashboard')); ?>";

function invoiceUrl(transactionId) {
    return <?php echo json_encode(url('/member/subscription/invoice'), 15, 512) ?> + '/' + encodeURIComponent(String(transactionId));
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    <?php if(session('renewal_blocked')): ?>
...
async function startRazorpayCheckout() {
    ...
    try {
        const res = await fetch("<?php echo e(route('member.subscription.order')); ?>", {
            method: 'POST',
            headers: {
        ...
        const options = {
            ...
            handler: async function (response) {
                ...
                try {
                    const verifyRes = await fetch("<?php echo e(route('member.subscription.verify')); ?>", {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name=\"csrf-token\"]').getAttribute('content'),
            ...
            prefill: {
                ...
                contact: <?php echo json_encode($user->mobile ?? '', 15, 512) ?>
            },
            theme: {
                color: '#351c42'
            },
...
<?php echo $__env->make('member.layouts.gnat', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\acer\OneDrive\Desktop\projects\event-management\resources\views/member/subscription/index.blade.php ENDPATH**/ ?>