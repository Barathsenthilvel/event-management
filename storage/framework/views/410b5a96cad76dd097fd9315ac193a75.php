...
<article class="overflow-hidden rounded-2xl border border-[#351c42]/10 bg-white shadow-sm">
    <div class="grid gap-6 p-4 sm:p-6 md:grid-cols-[minmax(0,280px)_1fr] md:items-stretch">
        <div class="relative min-h-[12rem] overflow-hidden rounded-2xl border border-[#351c42]/10 bg-[#f6f3e9] md:min-h-[14rem]">
            <img src="<?php echo e($cover); ?>" alt="<?php echo e($event->title); ?>" class="h-full w-full object-cover" loading="lazy" />
            <div class="absolute left-3 top-3 rounded-full bg-[#fddc6a] px-3 py-2 text-center shadow-sm">
                ...
                <div class="mt-0.5 inline-block rounded bg-white/70 px-2 py-0.5 text-[10px] font-extrabold tracking-widest text-[#965995]"><?php echo e($month); ?></div>
            </div>
        </div>
        ...
        <div class="flex min-w-0 flex-col">
            <div class="flex flex-wrap items-start justify-between gap-3">
                ...
                <div class="shrink-0 rounded-full border border-[#351c42]/15 bg-white px-3 py-1.5 text-right shadow-sm" aria-label="<?php echo e($seatLimited ? 'Limited seats' : 'Unlimited seats'); ?>">
                    <?php if($seatLimited): ?>
                        <?php if($seatsFull): ?>
                        ...
                        <?php else: ?>
                            ...
                            <div class="mt-0.5 text-xs font-extrabold tabular-nums text-[#351c42]"><?php echo e($filled); ?> / <?php echo e($seatCap > 0 ? $seatCap : '—'); ?></div>
                        <?php endif; ?>
                    <?php else: ?>
            ...
            <p class="mt-1 text-[11px] font-bold uppercase tracking-wide text-[#965995]"><?php echo e(strtoupper((string) $event->status)); ?></p>
            <h3 class="mt-1 text-base font-bold text-[#351c42] sm:text-lg"><?php echo e($event->title); ?></h3>
            <p class="mt-2 text-sm leading-relaxed text-[#351c42]/80"><?php echo e(Str::limit($desc, 220)); ?></p>
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-2">
                <div class="rounded-2xl border border-[#351c42]/10 bg-[#f6f3e9] p-4">
                    ...
                    <p class="mt-2 text-sm font-bold text-[#351c42]"><?php echo e($organizer); ?></p>
                </div>
                <div class="rounded-2xl border border-[#351c42]/10 bg-[#f6f3e9] p-4">
                    <div class="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-[#351c42]/70">
                        <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-[#965995]/15 text-[#965995]">
                    ...
                    <p class="mt-2 text-sm font-bold text-[#351c42]"><?php echo e($event->venue ?: 'Venue not specified'); ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <?php if($mode === 'tracking' && isset($invite)): ?>
        <?php $ps = $invite->participation_status; ?>
        <?php if($ps === 'participated'): ?>
            <div class="border-t border-[#351c42]/10 bg-[#f6f3e9] px-4 py-3 sm:px-6">
                <?php $certReady = ! empty($event->template_pdf_path); ?>
                <div class="flex flex-col items-center gap-2 text-center sm:flex-row sm:justify-between sm:text-left">
                    ...
                    <?php if($certReady): ?>
                        <a href="<?php echo e(route('member.events.certificate', $event)); ?>" class="inline-flex w-full items-center justify-center rounded-xl bg-[#351c42] px-4 py-2.5 text-sm font-bold text-[#fddc6a] transition hover:brightness-105 sm:w-auto">Download certificate</a>
                    <?php else: ?>
                        <span class="text-xs font-semibold text-[#351c42]/55">Certificate will be uploaded soon.</span>
                    <?php endif; ?>
                </div>
            </div>
        <?php elseif($ps === 'not_participated'): ?>
            <div class="border-t border-[#351c42]/10 bg-[#f6f3e9] px-4 py-3 sm:px-6">
        ...
        <?php else: ?>
            <div class="flex flex-wrap items-center gap-3 border-t border-[#351c42]/20 bg-[#351c42] px-4 py-3 sm:px-6 rounded-b-2xl">
                <?php echo $__env->make('member.partials.member-event-interested-stack', ['event' => $event], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    ...
    <?php else: ?>
        ...
        ?>
        <?php if($alreadyInterested && $myInvite): ?>
            <?php if($myInvite->participation_status === 'participated'): ?>
                <div class="border-t border-[#351c42]/10 bg-[#f6f3e9] px-4 py-3 sm:px-6">
                    <?php $certReadyList = ! empty($event->template_pdf_path); ?>
                    <div class="flex flex-col items-center gap-2 sm:flex-row sm:justify-between">
                        <span class="text-sm font-extrabold text-[#351c42]/85">Attended</span>
                        <?php if($certReadyList): ?>
                            <a href="<?php echo e(route('member.events.certificate', $event)); ?>" class="text-sm font-bold text-[#965995] underline-offset-2 hover:text-[#351c42] hover:underline">Download certificate</a>
                        <?php else: ?>
                            <span class="text-xs font-semibold text-[#351c42]/55">Certificate coming soon</span>
                        <?php endif; ?>
                    </div>
                </div>
            <?php elseif($myInvite->participation_status === 'not_participated'): ?>
                <div class="border-t border-[#351c42]/10 bg-[#f6f3e9] px-4 py-3 sm:px-6">
                    <p class="text-center text-sm font-extrabold text-[#351c42]/75">Did not attend</p>
                </div>
            <?php else: ?>
        ...
        <?php elseif($seatsFull): ?>
            <div class="rounded-b-2xl border-t border-rose-200 bg-rose-50 px-4 py-3 sm:px-6">
                ...
                <p class="mt-1 text-center text-xs font-semibold text-rose-800/90">Seat limit reached (<?php echo e($filled); ?> / <?php echo e($seatCap); ?>).</p>
            </div>
        ...
        <?php else: ?>
            <div class="flex flex-wrap items-center gap-3 border-t border-[#351c42]/20 bg-[#351c42] px-4 py-3 sm:px-6 rounded-b-2xl">
                <?php echo $__env->make('member.partials.member-event-interested-stack', ['event' => $event], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                <form method="POST" action="<?php echo e(route('member.events.interest', $event)); ?>" class="shrink-0 sm:ml-auto" onsubmit="this.querySelector('button[type=submit]')?.setAttribute('disabled','disabled')">
                    <?php echo csrf_field(); ?>
                    <button
                        type="submit"
...
<?php /**PATH C:\Users\acer\OneDrive\Desktop\projects\event-management\resources\views\member\partials\member-event-card.blade.php ENDPATH**/ ?>