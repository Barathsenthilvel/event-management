    ...
    <aside :class="sidebarOpen ? 'w-64' : 'w-20'"
        ...
        <nav class="flex-1 px-3 space-y-1 overflow-y-auto custom-scroll">
            ...
            <a href="<?php echo e(route('member.dashboard')); ?>"
                class="flex items-center gap-3 p-3 <?php echo e(request()->routeIs('member.dashboard') ? 'nav-item-active' : 'text-slate-400 hover:text-white hover:bg-white/5'); ?> rounded-xl transition-all group">
                <svg class="w-5 h-5 <?php echo e(request()->routeIs('member.dashboard') ? 'text-indigo-400' : 'group-hover:text-indigo-400'); ?> transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="<?php echo e(route('member.ebooks.index')); ?>"
                class="flex items-center gap-3 p-3 <?php echo e(request()->routeIs('member.ebooks.*') ? 'nav-item-active' : 'text-slate-400 hover:text-white hover:bg-white/5'); ?> rounded-xl transition-all group">
                <svg class="w-5 h-5 <?php echo e(request()->routeIs('member.ebooks.*') ? 'text-indigo-400' : 'group-hover:text-indigo-400'); ?> transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <?php if($canSeeMembership && $hasActiveSubscription): ?>
                <a href="<?php echo e(route('member.subscription.index')); ?>"
                    class="flex items-center gap-3 p-3 <?php echo e(request()->routeIs('member.subscription.*') ? 'nav-item-active' : 'text-slate-400 hover:text-white hover:bg-white/5'); ?> rounded-xl transition-all group">
                    <svg class="w-5 h-5 <?php echo e(request()->routeIs('member.subscription.*') ? 'text-indigo-400' : 'group-hover:text-indigo-400'); ?> transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="<?php echo e(route('member.profile.edit')); ?>"
                class="flex items-center gap-3 p-3 <?php echo e(request()->routeIs('member.profile.*') ? 'nav-item-active' : 'text-slate-400 hover:text-white hover:bg-white/5'); ?> rounded-xl transition-all group">
                <svg class="w-5 h-5 <?php echo e(request()->routeIs('member.profile.*') ? 'text-indigo-400' : 'group-hover:text-indigo-400'); ?> transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="p-4 border-t border-white/5 bg-slate-800/50 rounded-b-[24px] shrink-0">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-lg bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center text-xs font-bold text-indigo-300">
                    <?php echo e(strtoupper(substr(Auth::user()->name ?? 'ME', 0, 2))); ?>

                </div>
                <div x-show="sidebarOpen">
                    <p class="text-xs font-bold text-white"><?php echo e(Auth::user()->name); ?></p>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="hidden md:block">
                        ...
                        <div class="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                            ...
                            <span class="text-indigo-600"><?php echo e(request()->routeIs('member.profile.*') ? 'Profile' : 'Dashboard'); ?></span>
                        </div>
                    </div>
                </div>
    ...
    <div x-show="showLogout" x-cloak
        ...
        <div class="bg-white rounded-[40px] p-10 max-w-sm w-full text-center shadow-2xl"
            ...
            <div class="flex gap-3">
                ...
                <form method="POST" action="<?php echo e(route('member.logout')); ?>" class="flex-1">
                    <?php echo csrf_field(); ?>
                    <button type="submit"
...
<?php /**PATH C:\Users\acer\OneDrive\Desktop\projects\event-management\resources\views\member\layouts\app.blade.php ENDPATH**/ ?>