...
<div class="h-full flex flex-col p-6 gap-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 px-6 py-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex shrink-0 justify-end">
            <a href="<?php echo e(route('admin.home-banners.create')); ?>"
            ...
            <?php $__empty_1 = true; $__currentLoopData = $banners; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $banner): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr>
                    <td class="px-6 py-4 align-middle">
                        <img
                            src="<?php echo e(asset('storage/' . ltrim((string) $banner->image_path, '/'))); ?>"
                            alt=""
                            class="h-14 w-24 rounded-lg border border-slate-200 object-cover"
                        >
                    </td>
                    <td class="px-6 py-4 align-middle">
                        <p class="text-[11px] font-semibold text-slate-900"><?php echo e($banner->title ?: ($banner->caption_title ?: 'Untitled banner')); ?></p>
                        <?php if($banner->eyebrow): ?>
                            <p class="text-[10px] mt-1 font-bold uppercase tracking-wide text-indigo-600"><?php echo e($banner->eyebrow); ?></p>
                        <?php endif; ?>
                    </td>
                    <td class="px-6 py-4 align-middle">
                        <p class="font-semibold text-[11px] text-slate-700"><?php echo e($banner->link_url ?: '#'); ?></p>
                    </td>
                    ...
                    <td class="px-6 py-4 align-middle">
                        <p class="text-[11px] font-bold text-slate-500"><?php echo e($banner->updated_at?->format('d M Y')); ?></p>
                        <p class="font-semibold text-[11px] text-slate-700"><?php echo e($banner->creator->name ?? 'Admin'); ?></p>
                    </td>
                    <td class="px-6 py-4 text-center align-middle">
                        <form method="POST" action="<?php echo e(route('admin.home-banners.toggle-status', $banner->id)); ?>" class="inline-flex">
                            <?php echo csrf_field(); ?>
                            <button title="Toggle Display Status" type="submit" class="inline-flex items-center cursor-pointer">
                                <span class="w-10 h-5 <?php echo e($banner->is_active ? 'bg-emerald-400/60' : 'bg-slate-300/70'); ?> rounded-full relative shadow-inner">
                                    <span class="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform <?php echo e($banner->is_active ? 'translate-x-5' : ''); ?>"></span>
                                </span>
                            </button>
                        </form>
                    </td>
                    <td class="px-6 py-4 text-right align-middle">
                        <div class="inline-flex items-center justify-end gap-2">
                            <a href="<?php echo e(route('admin.home-banners.edit', $banner->id)); ?>"
                               title="Modify"
                            ...
                            <form id="admin-delete-home-banner-<?php echo e($banner->id); ?>" method="POST" action="<?php echo e(route('admin.home-banners.destroy', $banner->id)); ?>" class="inline-flex">
                                <?php echo csrf_field(); ?>
                                <?php echo method_field('DELETE'); ?>
                                <button type="button" title="Delete"
                                        data-delete-form="admin-delete-home-banner-<?php echo e($banner->id); ?>"
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\acer\OneDrive\Desktop\projects\event-management\resources\views/admin/home-banners/index.blade.php ENDPATH**/ ?>