    ...
    <div class="relative z-10 w-full max-w-[520px] max-h-[min(92vh,720px)] overflow-y-auto rounded-[28px] border border-[#fddc6a]/20 shadow-[0_24px_80px_-12px_rgba(0,0,0,0.55)] bg-gradient-to-b from-[#2d1b36] via-[#26152f] to-[#1a0f22] p-6 sm:p-8">
        ...
        <?php if(auth()->guard()->check()): ?>
            <div class="mt-5 rounded-2xl border border-[#fddc6a]/25 bg-white/5 px-4 py-3 text-sm text-white/90">
                <span class="font-semibold text-[#fcd34d]">Signed in as <?php echo e(auth()->user()->name); ?></span>
        ...
        <?php else: ?>
            <div id="donate-step-details" class="mt-6 space-y-4">
                <div class="rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm leading-relaxed text-white/85">
                    ...
                    <p class="mt-1 text-white/70">Please <a href="<?php echo e(route('member.login', ['return' => url()->current()])); ?>" class="font-bold text-white underline-offset-2 hover:underline">log in and pay</a> so we can link your donation to your profile.</p>
                    <p class="mt-3 font-semibold text-white/90">Paying as a guest?</p>
                    <p class="mt-1 text-white/65">Enter your details below, then choose an amount. Interested in membership? <a href="<?php echo e(route('member.register')); ?>" class="font-bold text-[#fcd34d] underline-offset-2 hover:underline">Sign up here</a>.</p>
                </div>
                <div class="space-y-3">
                    <label class="block">
        ...
        <div id="donate-step-amounts-wrapper" class="<?php if(auth()->guard()->guest()): ?> hidden <?php endif; ?>">
        <div id="modal-donate-amounts" class="mt-7 space-y-5">
            <div class="flex flex-col gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <?php $__currentLoopData = $dm['amounts'] ?? [10, 25, 50, 100, 250]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $amt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <button
                            type="button"
                            data-donate-amt="<?php echo e($amt); ?>"
                            class="donate-amt-btn rounded-full bg-white/[0.07] hover:bg-white/15 px-4 py-2.5 text-sm font-bold border border-white/20 text-white transition-colors <?php echo e((int) $amt === (int) ($dm['default_amount'] ?? 100) ? 'is-selected' : ''); ?>"
            ...
            <div>
                <div class="h-2.5 rounded-full bg-black/40 overflow-hidden ring-1 ring-white/5">
                    <div class="donate-progress-bar h-full rounded-full bg-[#fcd34d] transition-all duration-500 shadow-[0_0_12px_rgba(252,211,77,0.35)]"
                         style="width: <?php echo e((int) ($dm['bar_percent_demo'] ?? 52)); ?>%;"
                         data-donate-bar></div>
                </div>
            ...
            <div class="flex flex-col gap-3 pt-1">
                ...
                <label class="relative flex-1 flex items-center rounded-2xl bg-white pl-12 pr-4 py-3.5 shadow-inner ring-1 ring-black/5">
                    ...
                    <input type="number" min="1" step="1" value="<?php echo e((int) ($dm['default_amount'] ?? 100)); ?>" data-donate-input class="w-full min-w-0 border-0 bg-transparent text-[#351c42] text-lg font-bold outline-none focus:ring-0" />
                </label>
...
<?php /**PATH C:\Users\acer\OneDrive\Desktop\projects\event-management\resources\views/home/partials/donate-modal.blade.php ENDPATH**/ ?>