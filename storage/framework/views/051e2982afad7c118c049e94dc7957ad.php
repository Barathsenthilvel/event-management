...
<section id="member-pollings" class="scroll-mt-28 space-y-6 rounded-2xl border border-[#351c42]/10 bg-white/90 p-5 shadow-md sm:p-6">
    ...
    <?php if($showVoteSuccessBanner): ?>
        <div class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-900" role="status"><?php echo e(session('polling_success')); ?></div>
    <?php endif; ?>
    <?php if(session('polling_error')): ?>
        <div class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-800" role="alert"><?php echo e(session('polling_error')); ?></div>
    <?php endif; ?>

    <?php $__empty_1 = true; $__currentLoopData = $memberPollings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $poll): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <?php
        ...
        <article class="rounded-2xl border border-[#351c42]/10 bg-[#f6f3e9] p-4 sm:p-5">
            <h3 class="text-lg font-bold text-[#351c42]"><?php echo e($poll->title); ?></h3>
            <div class="mt-1 flex flex-wrap items-center gap-2">
                <p class="text-xs font-semibold text-[#351c42]/55">
                    <?php if($poll->polling_date_to && $poll->polling_date_to->toDateString() !== $poll->polling_date->toDateString()): ?>
                        <?php echo e($poll->polling_date?->format('d M Y')); ?> – <?php echo e($poll->polling_date_to->format('d M Y')); ?>
            ...
            <ul class="mt-4 space-y-5">
                <?php $__currentLoopData = $poll->positions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ppos): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
            ...
            <?php if(isset($pollingResultStats[$poll->id]) && $pollingResultStats[$poll->id] !== []): ?>
                <div class="mt-6 space-y-4 rounded-2xl border border-emerald-200/80 bg-emerald-50/60 p-4">
                    ...
                    <?php $__currentLoopData = $poll->positions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ppos): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $stats = $pollingResultStats[$poll->id][$ppos->id] ?? null; ?>
                        <?php if(!$stats) continue; ?>
                        <div class="rounded-xl border border-white/60 bg-white/90 p-3">
                            <p class="text-sm font-extrabold text-[#351c42]"><?php echo e($ppos->position); ?></p>
                            <?php if(! empty($stats['winner_name'])): ?>
                                <p class="mt-1 text-xs text-[#351c42]/80">Official winner: <strong class="text-[#351c42]"><?php echo e($stats['winner_name']); ?></strong></p>
                            <?php endif; ?>
                            <ul class="mt-3 space-y-2">
                                <?php $__currentLoopData = $stats['candidates']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li class="relative overflow-hidden rounded-xl border border-[#351c42]/10 bg-[#faf9fc]">
                                        <div class="pointer-events-none absolute inset-y-0 left-0 rounded-xl bg-gradient-to-r from-emerald-200/80 to-emerald-100/40" style="width: <?php echo e($row['bar_percent']); ?>%"></div>
                                        <div class="relative flex items-center justify-between gap-2 px-3 py-2 text-xs">
                                            <span class="font-bold text-[#351c42]"><?php echo e($row['name']); ?></span>
                                            <span class="tabular-nums font-extrabold text-[#351c42]"><?php echo e($row['votes']); ?></span>
                                        </div>
                                    </li>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </ul>
...
<?php /**PATH C:\Users\acer\OneDrive\Desktop\projects\event-management\resources\views\member\partials\member-pollings-panel.blade.php ENDPATH**/ ?>