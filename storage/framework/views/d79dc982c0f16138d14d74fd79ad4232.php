...
<div x-data="rolesPage()" data-destroy-url="<?php echo e(url('/admin/roles')); ?>" data-can-manage-menu-permissions="<?php echo e(($canManageMenuPermissions ?? false) ? '1' : '0'); ?>" class="h-full flex gap-3 workspace-transition relative p-6" :class="showAddPanel ? '' : ''">
<div class="flex flex-col gap-3 workspace-transition flex-1" :class="showAddPanel ? 'w-2/3' : 'w-full'">
    ...
    <div id="roles-list-content" class="flex-1 overflow-y-auto custom-scroll pr-2 pb-4">
            ...
            <tbody class="text-xs">
                ...
                <tr class="group transition-all">
                    <td
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-500 flex items-center justify-center font-bold">
                                <?php echo e(strtoupper(substr($role->name, 0, 1))); ?></div>
                            <div>
                                <p class="font-bold text-slate-800"><?php echo e($role->name); ?></p>
                                <div class="flex items-center gap-1 mt-0.5">
                                    <?php if($roleNoModules): ?>
                                        <span class="inline-flex rounded-md border border-amber-200 bg-amber-50 px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wide text-amber-800">Permission not provided</span>
                                    <?php else: ?>
                    ...
                    <td class="px-6 py-4 bg-white border-y border-slate-100 text-slate-400 font-medium">
                        <?php echo e($role->created_at->format('d M Y')); ?></td>
                    ...
                    <td
                        ...
                        <div class="flex items-center justify-end gap-1">
                            <?php if($canManageMenuPermissions ?? false): ?>
                            <button type="button" data-action="permissions" data-role-id="<?php echo e($role->id); ?>" data-role-name="<?php echo e(e($role->name)); ?>"
                            ...
                            <?php endif; ?>
                            <button type="button" data-action="edit" data-role-id="<?php echo e($role->id); ?>"
                                class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg"
                                title="Edit Role">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"
                                    viewBox="0 0 24 24">
                            ...
                            <button type="button" data-action="delete" data-role-id="<?php echo e($role->id); ?>" data-role-name="<?php echo e(e($role->name)); ?>"
                ...
                <tr>
                    <td colspan="4" class="px-6 py-8 text-center text-slate-400">
                        <p>No roles found. <a href="<?php echo e(route('admin.roles.create')); ?>" class="text-indigo-600 hover:underline">Create one</a></p>
                    </td>
                </tr>
                <?php endif; ?>
            </tbody>
        ...
        <?php if($roles->hasPages()): ?>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Showing <?php echo e($roles->firstItem()); ?> to <?php echo e($roles->lastItem()); ?> of <?php echo e($roles->total()); ?> Roles</p>
            <div class="flex gap-1">
                <?php echo e($roles->links('pagination.simple-tailwind')); ?>

            </div>
        <?php else: ?>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Showing <?php echo e($roles->count()); ?> Roles</p>
        <?php endif; ?>
        </div>
        ...
        <div x-show="viewType === 'grid'" x-cloak
            ...
            <div
                ...
                <h4 class="font-bold text-slate-800 mb-1"><?php echo e($role->name); ?></h4>
                <?php if($gridNoModules): ?>
                    <p class="mb-2 text-[10px] font-black uppercase text-amber-700">Permission not provided</p>
                <?php endif; ?>
                <p class="text-xs text-slate-400 mb-4 line-clamp-2"><?php echo e($role->description ?? 'Manages access levels and permissions for this specific user group.'); ?></p>
                ...
                <div class="mt-4 flex gap-2 pt-4 border-t border-slate-50">
                    ...
                    <?php endif; ?>
                    <button type="button" data-action="edit" data-role-id="<?php echo e($role->id); ?>" class="p-2 bg-slate-50 text-slate-400 rounded-xl hover:text-indigo-600" title="Edit"><svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
                    <button type="button" data-action="delete" data-role-id="<?php echo e($role->id); ?>" data-role-name="<?php echo e(e($role->name)); ?>" class="p-2 bg-slate-50 text-slate-400 rounded-xl hover:text-rose-600" title="Delete"><svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                </div>
            </div>
    ...
    <div x-show="showAddPanel" x-cloak
        ...
        <form method="POST" action="<?php echo e(route('admin.roles.store')); ?>" @submit.prevent="submitCreateRole($event.target)" class="p-8 space-y-6 flex-1 overflow-y-auto custom-scroll">
            <?php echo csrf_field(); ?>
            <div>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\acer\OneDrive\Desktop\projects\event-management\resources\views\admin\roles\index.blade.php ENDPATH**/ ?>