...
<div class="h-full flex flex-col p-6 gap-3" x-data="defaultSettingsPage()">
    ...
    <div class="bg-white flex-1 rounded-[24px] shadow-sm flex flex-col p-6 overflow-hidden">
        ...
        <div class="mb-8 p-6 bg-slate-50/50 rounded-2xl border border-slate-100">
            ...
            <form action="<?php echo e(route('admin.settings.default-settings.store')); ?>" method="POST" class="space-y-4">
                <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div>
                        ...
                        <select name="currency_code" required x-model="selectedCurrency"
                            ...
                            <?php $__currentLoopData = config('countries_currencies.currencies', []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $code => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="time_format" required
                            class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 appearance-none cursor-pointer">
                            <?php $__currentLoopData = config('countries_currencies.time_formats', []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <tbody class="text-xs">
                        ...
                        <tr class="group transition-all">
                            <td class="px-6 py-4 bg-white border-y border-l border-slate-100 first:rounded-l-2xl">
                                <span class="font-bold text-slate-800"><?php echo e($s->country_name); ?></span>
                                <span class="text-slate-400 ml-1">(<?php echo e($s->country_code); ?>)</span>
                            </td>
                            <td class="px-6 py-4 bg-white border-y border-slate-100">
                                <span class="font-medium text-slate-700"><?php echo e($s->currency_code); ?></span>
                                <?php if($s->currency_name): ?>
                                    <span class="text-slate-400"> - <?php echo e($s->currency_name); ?></span>
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4 bg-white border-y border-slate-100 text-slate-600"><?php echo e($s->time_format); ?></td>
                            <td class="px-6 py-4 bg-white border-y border-slate-100 text-center">
                                ...
                                <?php else: ?>
                                    <form action="<?php echo e(route('admin.settings.default-settings.set-default', $s)); ?>" method="POST" class="inline">
                                        <?php echo csrf_field(); ?>
                            ...
                            <td class="px-6 py-4 bg-white border-y border-r border-slate-100 last:rounded-r-2xl text-right">
                                <form id="admin-delete-default-setting-<?php echo e($s->id); ?>" action="<?php echo e(route('admin.settings.default-settings.destroy', $s)); ?>" method="POST" class="inline">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('DELETE'); ?>
                                    <button type="button" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg" title="Delete"
                                        data-delete-form="admin-delete-default-setting-<?php echo e($s->id); ?>"
...
function defaultSettingsPage() {
    const countries = <?php echo json_encode(config('countries_currencies.countries', []), 512) ?>;
    return {
        countries,
        countrySearch: '',
        countryOpen: false,
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\acer\OneDrive\Desktop\projects\event-management\resources\views\admin\settings\default-settings.blade.php ENDPATH**/ ?>